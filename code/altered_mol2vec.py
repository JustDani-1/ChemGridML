...
import warnings     # To suppress deprecation warnings for Morgan Fingerprint 
...
def sentences2vec(sentences, model, unseen=None):
    ...
    np.array
    """
    # Replace model.wv.vocab.keys() with model.wv.key_to_index.keys()
    keys = set(model.wv.key_to_index.keys())
    vec = []
    if unseen:
        unseen_vec = model.wv[unseen]  # Also updated this line for consistency
        for sentence in sentences:
            vec.append(sum([model.wv[y] if y in set(sentence) & keys
                           else unseen_vec for y in sentence]))
    else:
        for sentence in sentences:
            vec.append(sum([model.wv[y] for y in sentence
